...
In addition to the public API, DaWanda has an OAuth API to securely provide access to user-specific data. For further information on what OAuth is and how it works see [the official OAuth website|http://oauth.net/].

For using the OAuth API you have to request a consumer key and consumer secret at [http://dawanda.com/apps].
...
DaWanda will redirect the user back to a given callback URL, where you now can create an 'Access Token' to make API requests.
  ...
  >> Dawanda::OAuth.connect(OAUTH_KEY, OAUTH_SECRET, params)
